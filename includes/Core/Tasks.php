...
?>
	...
	public static function new_task_row($task, $frontend = false, $filters = []) {
		...
		if ($canView) {
			...
			ob_start(); ?>
			<a href="<?php echo esc_url($task_url); ?>" class="zpm_task_list_row <?php echo esc_attr($row_classes); ?>" data-project-url="<?php echo $projectURL; ?>" data-task-id="<?php echo esc_attr($task->id); ?>" ripple="ripple" data-ripple="rgba(0,0,0,0.1)" data-task-name="<?php echo esc_html($task->name); ?>" id="zpm-task-list__item-<?php echo esc_attr($task->id); ?>" data-due="<?php echo $task->date_due; ?>">
				...
				<span class="zpm_task_details">
					<?php if ($project_name !== '' && !zpm_is_single_project()) : ?>
						<span data-zpm-link="<?php echo $projectURL; ?>" title="<?php esc_html_e('Project', 'zephyr-project-manager'); ?>" class="zpm_task_project"><?php echo esc_html($project_name); ?></span>
					<?php endif; ?>
					...
					<label for="zpm-task-bulk-selector-<?php echo esc_attr($task->id); ?>" class="zpm-material-checkbox zpm-task-bulk-selector-wrap">
						<input type="checkbox" id="zpm-task-bulk-selector-<?php echo esc_attr($task->id); ?>" name="zpm-task-bulk-selector-<?php echo esc_attr($task->id); ?>" class="zpm-task-bulk-selector zpm_toggle invisible" value="1" data-task-bulk-selector="<?php echo esc_attr($task->id); ?>">
						<span class="zpm-material-checkbox-label"></span>
					</label>
				</span>
	...
	public static function get_assignees($task, $get_object = false) {
		...
		if (!property_exists($task, 'assignee')) return [];
		...
		<div class="zpm_subtask_item <?php echo $task->isCompleted() ? 'zpm_task_complete' : ''; ?> <?php echo $hasChildren ? 'has-children' : ''; ?> <?php echo $level > 0 ? "zpm-sub-subtask zpm-subtask-item--sub zpm-subtask-level-{$level}" : ''; ?> <?php echo esc_attr($classes); ?>" data-zpm-subtask="<?php echo esc_attr($task->id); ?>">
			<div class="zpm-subtask-item--content">
				...
				<div class="zpm-subtask-actions">
					<span data-zpm-subtask-id="<?php echo esc_attr($task->id); ?>" class="zpm_update_subtask"><?php esc_html_e('Save Changes', 'zephyr-project-manager'); ?></span>
					<?php if (Utillities::canDeleteTask(get_current_user_id(), $task)): ?>
						<span data-zpm-subtask-id="<?php echo esc_attr($task->id); ?>" class="zpm_delete_subtask"><?php esc_html_e('Delete', 'zephyr-project-manager'); ?></span>
					<?php endif; ?>
					<?php if ($level == 0): ?>
	...
	?>
		<div id="zpm_new_subtask_modal" class="zpm-modal zpm_compact_modal" aria-modal="true" aria-hidden="true">
			<div class="zpm-form__group">
				<input type="text" name="zpm_new_subtask_name" id="zpm_new_subtask_name" class="zpm-form__field" placeholder="<?php esc_html_e('Subtask Name', 'zephyr-project-manager'); ?>" data-ajax-name="name" />
				<label for="zpm_new_subtask_name" class="zpm-form__label"><?php esc_html_e('Subtask Name', 'zephyr-project-manager'); ?></label>
			</div>

			<div class="zpm-form__group">
				<textarea type="text" name="zpm-new-subtask__description" id="zpm-new-subtask__description" class="zpm-form__field" placeholder="<?php esc_html_e('Subtask Description', 'zephyr-project-manager'); ?>" data-ajax-name="description"></textarea>
				<label for="zpm-new-subtask__description" class="zpm-form__label"><?php esc_html_e('Subtask Description', 'zephyr-project-manager'); ?></label>
			</div>

			<div class="zpm-row">
				<div class="zpm-col zpm-col-6">
					<div class="zpm-form__group">
						<input type="text" name="zpm-new-subtask__start" id="zpm-new-subtask__start" class="zpm-form__field zpm-datepicker" placeholder="<?php esc_html_e('Start', 'zephyr-project-manager'); ?>" data-ajax-name="start-date" />
						<label for="zpm-new-subtask__start" class="zpm-form__label"><?php esc_html_e('Start', 'zephyr-project-manager'); ?></label>
					</div>
				</div>
				<div class="zpm-col zpm-col-6">
					<div class="zpm-form__group">
						<input type="text" name="zpm-new-subtask__due" id="zpm-new-subtask__due" class="zpm-form__field zpm-datepicker" placeholder="<?php esc_html_e('Due', 'zephyr-project-manager'); ?>" data-ajax-name="due-date" />
						<label for="zpm-new-subtask__due" class="zpm-form__label"><?php esc_html_e('Due', 'zephyr-project-manager'); ?></label>
					</div>
				</div>
			</div>

			<button id="zpm_save_new_subtask" class="zpm_button"><?php esc_html_e('Create Subtask', 'zephyr-project-manager'); ?></button>
		</div>
	<?php
	}
	...
	public static function contextMenu($taskID) {
		...
		ob_start(); ?>
		<span class="zpm_modal_options_btn zpm-task-context-menu" data-dropdown-id="zpm_view_task_dropdown" data-task-id="<?php echo esc_attr($taskID); ?>" aria-haspopup="true">
			...
			<div class="zpm_modal_dropdown" id="zpm_view_task_dropdown" aria-hidden="true">
				<ul class="zpm_modal_list">
					<?php if (Utillities::canDeleteTask(get_current_user_id(), $task)) : ?>
						...
						<li id="zpm_archive_task" data-archive-task="<?php echo esc_attr($taskID) ?>" data-task-id="<?php echo esc_attr($taskID) ?>" data-archived="<?php echo esc_attr($task->archived) ?>"><?php echo $task->archived == '1' ? __('Unarchive Task', 'zephyr-project-manager') : __('Archive Task', 'zephyr-project-manager'); ?></li>
					<?php endif; ?>
...
";
	...
	?>
		<div class="zpm-form__group zpm-new-task-field__name">
			<input type="text" name="zpm_new_task_name" id="zpm_new_task_name" class="zpm-form__field" data-task-list-item placeholder="<?php esc_html_e('Enter Task Name', 'zephyr-project-manager'); ?>" autocomplete="off">
			<label for="zpm_new_task_name" class="zpm-form__label"><?php esc_html_e('Add Task Name', 'zephyr-project-manager'); ?></label>
		</div>
<?php
		return ob_get_clean();
	}
