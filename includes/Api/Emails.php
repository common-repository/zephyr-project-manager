...
<?php endforeach;
	...
	public static function new_subtask_email($subtask, $user_id = null) {
		...
		foreach ($members as $member) {
			...
			if (!Emails::checkPreference($member, 'new_subtask', ['task' => $subtask])) continue;
		...
		if ($content !== '') {
			...
			if ($type == 'task') {
				...
				foreach ($assignees as $assignee) {
					...
					$preference = Emails::checkPreference($member, 'task_comment', [
						'task' => $object
					]);
				...
				foreach ($members as $member) {
					...
					if (in_array($member['email'], $sent_emails)) continue;
					if (!$settings['override_default_emails']) continue;
	...
	public static function checkPreference($member, $email, $args = []) {
		if ($email == 'task_comment') {
			if (Tasks::hasProject($args['task'])) {
				...
				if (!$projectsCommentsEmail) return false;
			}
		}
	...
	public static function assignedTaskEmail($task) {
		...
		foreach ($assigneeEmails as $member) {
			...
			if (!Emails::checkPreference($member, 'new_task', ['task' => $task])) continue;
	...
	public static function sendTaskDueEmail($task) {
		...
		foreach ($assignees as $assignee) {
			if (!Members::isNotificationEnabled($assignee, 'tasks')) continue;
